...
export default class MostViewedCity extends Component {
  render() {
    const { cities, className } = this.props;
    return (
      <>
        {cities.city.length !== 0 && cities.city
                .filter(view => view.visit_count >= 10).length !== 0 && (
          <div className="mt-4">
            ...
            <div className="row">
              {cities.city
                .filter(view => view.visit_count >= 10)
                .map((city, index) => {
                  return (
                    index < 3 && (
                      <div
                        className="col-md-6 col-lg-4 col-xl-3 mt-3 most-viewed-reveal"
                        key={index}
                      >
